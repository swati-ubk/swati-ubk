...
import { Button, StyleSheet, View, NativeModules, NativeEventEmitter, Text } from 'react-native';
...
export default class Payment extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      StoreID: this.props.route.params.data.StoreID,
      orderData: '',
      orderStatus: 'Success',
  ...
  fetchData = async () => {
    ...
    try {
      ...
      const response = await WebService.PostData(`business/${this.state.StoreID}/order`, requestOptions);
      const json = await response.json();
      console.log("==============order Response==================", json);
      if (json.hasOwnProperty('errors')) {
        console.log('error...', json.errors);
        this.props.navigation.navigate('FaildScreen', { data: json.errors });

      } else {
        this.setState({ data: json });
        console.log('paymentOption...', this.state.payment)
        if (this.state.payment == 'COD') {
          console.log('COD Loop', this.props);
          this.props.navigation.navigate('SuccessScreen', { data: json });

        } else {
          this.payment(json.apiKey, json.result.amount, json);
        }
  ...
  async payment(key, amount, OrderdataResponse) {
    ...
    var options = {
      ...
      amount: amount,
      name: 'Diskounto',
      order_id: OrderdataResponse.result.id,
      prefill: {
    ...
    RazorpayCheckout.open(options).then((data) => {
      ...
      this.setState({ orderData: `${data.razorpay_payment_id}` })
      this.props.navigation.navigate('SuccessScreen', { data: OrderdataResponse });
      //this.PaymentSuccess();
    }).catch((error) => {
      // handle failure
      console.log('error', error);
      this.setState({ orderData: `${error.code} | ${error.description}` })
      // this.PaymentFaild();
