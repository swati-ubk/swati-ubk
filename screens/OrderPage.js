...
import { View, useWindowDimensions, Text, StyleSheet, FlatList, SafeAreaView, Image, ScrollView, TouchableOpacity, ActivityIndicator } from 'react-native';
...
export default function TabViewExample({ navigation }) {
  ...
  useEffect(() => {
    ...
    async function fetchMyAPI() {
      try {
        ...
        WebService.PostData('my-orders?limit=100&skip=0', requestOptions).then(res => res.json()).then(resJson => {
          // return resJson;
          console.log('Order Data ...............', resJson);
          setDataSource(resJson);

        }).catch(e => console.log(e));
        WebService.PostData('store/my-orders?limit=20&skip=0', requestOptions).then(res => res.json()).then(resJson => {
          // return resJson;
          console.log('Rewards  Data ...............', resJson);
  ...
  const OrderStatus = (Status) => {
    ...
    return (
      <View style={{ alignSelf: 'center', borderWidth: 1, padding: 5, borderColor: ColorBorder }}>
        <Text style={{ fontFamily: globalcolor.Font, color: Colortext }}>{StatusCode}</Text>
      </View>
    )
      ...
      >
        <View style={{ flex: 1, flexDirection: 'row', padding: 15 }} >
          <View style={{ flex: 0.6 }}>
            ...
            <Text style={[styles.OrderText, { fontSize: 15, fontWeight: 'bold' }]}> {item.orderId}</Text>
            <Text style={[styles.OrderText, { fontSize: 13 }]}> ₹ {item.pricing.customerToPay}</Text>
          </View>
          <View style={{ flex: 0.3 }}>
            {OrderStatus(item.orderState)}
          </View>
          <View style={{ flex: 0.1 }}>
            <FontAwesome style={{ alignSelf: 'flex-end' }} name="chevron-right" color={globalcolor.SeconderFontColor} size={20} />
          </View>
    ...
    return (
      ...
      >
        <View style={{ flex: 1, flexDirection: 'row', padding: 15 }} >
          <View style={{ flex: 0.6 }}>
            <Text>{Moment(item.createdAt).format('DD MMM YYYY')}</Text>
            <Text style={[styles.OrderText, { fontSize: 14, fontWeight: 'bold' }]}> {item.orderId}</Text>
