import React from 'react';
import {
  StyleSheet,
  SafeAreaView,
  FlatList,
  View,
  Image,
  TouchableOpacity,
  Text,
  ActivityIndicator,
  TextInput,
  ScrollView,
  Alert,
  LogBox,
} from 'react-native';
import {ConfigFile} from '../service/ConfigFile';
import {globalcolor} from '../style/globalcolor';
import {globalstyle} from '../style/globals.js';
import WebService from '../service/WebService';
import FontAwesome from 'react-native-vector-icons/FontAwesome';
import AsyncStorage from '@react-native-async-storage/async-storage';
import Animated from 'react-native-reanimated';
import BottomSheet from 'reanimated-bottom-sheet';
import Counter from 'react-native-counters';

export default class ProductListScreen extends React.Component {
  constructor(props) {
    super(props);
    this.onEndReachedCalledDuringMomentum = true;
    this.state = {
      data: [],
      refreshing: false,
      // StoreID: '605a0d38c98f1137023019dc',
      StoreID: this.props.route.params.StoreId,
      CatId: '',
      loading: false,
      offset: 0,
      loadmore: false,
      nodata: false,
      cartegorydata: [],
      cartData: [],
      localcart: [],
      clickdata: [],
      count: 0,
      totalPrice: 0,
      acceptsCOD: this.props.route.params.acceptsCOD,
      requireSlot: this.props.route.params.requireSlot,
      AddTocartLoader: false,
    };
  }

  async componentDidMount() {

    console.log("dsfwfdfdfd--------------------222222---------")
    this.importData();
    // this.localtocart()
    this.setState({data: []});
    this.setState({loading: true});

    this.fetchCats('didcall');

    this.bs = React.createRef();
    this.fall = new Animated.Value(1);

    this.focusListener = this.props.navigation.addListener('focus', () => {
      this.importData();
      this.setState({data: []});
      this.setState({loading: true});
      this.fetchCats('didcall');

      //Put your Data loading function here instead of my this.loadData()
    });
  }
  componentDidUpdate(prevProps, prevState) {
   // console.log("dsfwfdfdfd---------------------333333--------")
    let a = prevState.StoreID;
    let b = this.props.route.params.StoreId;
    if (a != b) {
      console.log('update is calling.....');
    
      this.setState({StoreID: this.props.route.params.StoreId,data: [],loading: true});
      this.fetchCats('didupdate');
    } else {
    }
  }
  
  renderInner = () => {
   
    return (
      <View style={styles.panel}>
        <View>
          <Text style={styles.panelTitle}>
            Customise {this.state.clickdata.name}{this.state.clickdata.id}
          </Text>
        </View>
        <FlatList
          data={this.state.clickdata.variants}
          ItemSeparatorComponent={this.ItemSeparator}
          renderItem={this.product_variants}
          keyExtractor={item => item.id.toString()}
        />
        <TouchableOpacity
          style={styles.panelButton}
          onPress={() => this.bs.current.snapTo(1)}>
          <Text style={styles.panelButtonTitle}>Cancel</Text>
        </TouchableOpacity>
      </View>
    );
  };

  renderHeader = () => (
    <View style={styles.header}>
      <View style={styles.panelHeader}>
        <View style={styles.panelHandle} />
      </View>
    </View>
  );

  /**************************Start Here Search Product*******************************/
  SearchProduct = seatchText => {
    console.log('seatchText........', seatchText.length);
    this.setState({refreshing: true});
    if (seatchText.length > 0) {
      console.log('StoreID............', this.state.StoreID);
      WebService.GetDataJSon(
        `business-details/${this.state.StoreID}/products?searchText=${seatchText}&limit=10&skip=0`,
      ).then(response => {
        if (response.length > 0) {
          console.log(JSON.stringify(response));
          this.setState({data: response, refreshing: false});
        } else {
          this.setState({data: [], refreshing: false, noserachItem: false});
        }
      });
    } else {
      this.getproductlist(this.state.StoreID, this.state.CatId);
    }
  };
  /**************************End  Here Search Product*******************************/

  cutAdd = async (product, count) => {
    console.log('sas.....', product);
    try {
      let data_obj;
      var i;
    

      const cartData = await AsyncStorage.getItem('addToCart');

      console.log('nooooooo', JSON.parse(cartData));

      if(count==0)
      {
        let data2 = JSON.parse(cartData);
        var indexx = data2.findIndex(obj => obj.id === product.id);
        data2.splice(indexx, 1);
        await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
      }
      else{

      if (cartData === null || cartData === undefined) {

        var veri_id = product.variants[0].id;
        var sellingPrice = parseInt(product.variants[0].sellingPrice);
        data_obj = [{
          name: product.name,
          variants: {[veri_id]: count},
          id: product.id,
          priduce_v: [
            {
              id: veri_id,
              price: sellingPrice,
              count: count,
            },
          ],
        }];

      await AsyncStorage.setItem("addToCart", JSON.stringify(data_obj));
    }
    else{

      var veri_id = product.variants[0].id;
      var sellingPrice = parseInt(product.variants[0].sellingPrice);
      data_obj = {
        name: product.name,
        variants: {[veri_id]: count},
        id: product.id,
        priduce_v: [
          {
            id: veri_id,
            price: sellingPrice,
            count: count,
          },
        ],
      };
  
      let data2 = JSON.parse(cartData);
    
      var indexx = data2.findIndex(obj => obj.id === product.id);
      //arrr.splice(0)

      if(indexx==-1)
      {
        data2.push(data_obj);
     
      }
      else{
        data2.splice(indexx, 1);
        data2.push(data_obj);
      
      }
      await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
    
    }
  }

     
     

      this.importData();
    } catch (e) {
      console.log(e);
    }
  };

  subCutAdd = async (product, data, count) => {
  //  this.setState({loadmore: true});
    try {
      let data_obj,userData;
      var i;


      const cartData = await AsyncStorage.getItem('addToCart');

      console.log('nooooooo', JSON.parse(cartData));
      var veri_id = product.item.id;
      if(count==0)
      {
        console.log("=========count=========0===","10001");
        let data2 = JSON.parse(cartData);
          var indexx = data2.findIndex(obj => obj.id === data.id);
          var arrr=data2[indexx].priduce_v;
          var v_obj=data2[indexx].variants;
          ///v_obj[[veri_id]] = count;

          console.log("sfdfdfdfdfd====",data2[indexx].variants)
          var indexx2 = arrr.findIndex(obj => obj.id === veri_id);
        

         
         // var v_obj = data2[indexx].variants;
         /// v_obj[[veri_id]] = count;
               // arrr[indexx].count = count;
        if (arrr.length == 1) {
          console.log("=========null============","0001"+arrr);
          await AsyncStorage.removeItem("addToCart");
        }
        else{
          console.log("=========null============"+veri_id,"00002"+JSON.stringify(v_obj));
          arrr.splice(indexx2, 1);

          delete v_obj[veri_id];
          console.log("=========null============","00002"+JSON.stringify(v_obj));
          data_obj = [{
            name: data.name,
            variants: v_obj,
            id: data.id,
            priduce_v: arrr,
          }];


          console.log("=========null============","00002"+JSON.stringify(data_obj));

          await AsyncStorage.setItem("addToCart", JSON.stringify(data_obj));
        }
        
      }
      else{
        if (cartData === null || cartData === undefined || cartData === "[]" ){

          console.log("=========null============","10002");

                   var sellingPrice = parseInt(product.item.sellingPrice);

                    var veri_id = product.item.id;

                        data_obj = [{
                        name: data.name,
                        variants: {[veri_id]: count},
                        id: data.id,
                        priduce_v: [
                          {
                            id: veri_id,
                            price: sellingPrice,
                            count: count,
                          },
                        ],
                      }];

                      ///console.log("=======11========",JSON.stringify(data_obj));
             await AsyncStorage.setItem("addToCart", JSON.stringify(data_obj));

            }
        else
            {

                // console.log("=======11========","--"+cartData);

                let data2 = JSON.parse(cartData);
                var veri_id = product.item.id;
                var sellingPrice = parseInt(product.item.sellingPrice);
           // console.log('nooooooo11111111', veri_id);
                userData = {
                    name: data.name,
                    variants: {[veri_id]: count},
                    id: data.id,
                    priduce_v: [
                      {
                        id: veri_id,
                        price: sellingPrice,
                        count: count,
                      },
                    ],
                  };

              var indexx = data2.findIndex(obj => obj.id === data.id);
             if(indexx==-1)
              {
        
                  //console.log("=======10004========",userData);
                 // console.log("=================00999========",userData);
                  data2.push(userData);

                 console.log("=================00999========",JSON.stringify(data2));
                await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
                 // await AsyncStorage.setItem("addToCart", JSON.stringify(data_obj));
              }
              else{
                ///console.log("=================ok========"+veri_id,userData);

                var arrr=data2[indexx].priduce_v;
                ///console.log("=================ok009========",arrr);

                //let arrr = userData.priduce_v;
                var indexx1 = arrr.findIndex(obj => obj.id === veri_id);
               // data2
               console.log("=================ok009========",indexx1);



               var v_obj = data2[indexx].variants;
                   v_obj[[veri_id]] = count;
                   //console.log("=================ok0091990========",v_obj);
               //           console.log('====98======',v_obj);

                    var objj = {
                            id: veri_id,
                            price: product.item.sellingPrice,
                            count: count,
                           };
            
                         // console.log("====99===",objj);
                       

               if(indexx1==-1)
               {
                //console.log("=================ok0091========",arrr[indexx]);
               /// data2.push(userData);
               arrr.push(objj)
               console.log("====991111===",arrr);

               data_obj = [{
                 name: data.name,
                 variants: v_obj,
                 id: data.id,
                 priduce_v: arrr,
               }];

                   //   data2.push(data_obj);
                     console.log("=================0099933========",JSON.stringify(data2));
                    await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
               }
               else
               {
               

                     console.log("====991111===",arrr);
                     arrr.splice(indexx1, 1);

                       arrr.push(objj)
                        console.log("====991111===",arrr);

                        data_obj = [{
                          name: data.name,
                          variants: v_obj,
                          id: data.id,
                          priduce_v: arrr,
                        }];

                       // data2.push(data_obj);
                    console.log("=================ok0091990========",JSON.stringify(data2));
                    await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
               }
               /// var indexx2 = data2.findIndex(obj => obj.id === data.id);


              }

        }



      }



      // const cartData = await AsyncStorage.getItem('addToCart');

      // console.log("sd222---------","--"+cartData);
      // //console.log("sd222---------","--"+data);

      // if (cartData === null || cartData === undefined || cartData === "[]" ){

      //   var sellingPrice = parseInt(product.item.sellingPrice);

      //     var veri_id = product.item.id;
      //         data_obj = [{
      //         name: data.name,
      //         variants: {[veri_id]: count},
      //         id: data.id,
      //         priduce_v: [
      //           {
      //             id: veri_id,
      //             price: sellingPrice,
      //             count: count,
      //           },
      //         ],
      //       }];

      //       console.log("=======11========",data_obj);
      //       await AsyncStorage.setItem("addToCart", JSON.stringify(data_obj));
      //     }
      //     else{
      //       console.log("sd222---------","--"+cartData);

      //       let data2 = JSON.parse(cartData);


      //       var veri_id = product.item.id;
      //       var sellingPrice = parseInt(product.item.sellingPrice);
      //      // console.log('nooooooo11111111', veri_id);
      //      userData = {
      //         name: data.name,
      //         variants: {[veri_id]: count},
      //         id: data.id,
      //         priduce_v: [
      //           {
      //             id: veri_id,
      //             price: sellingPrice,
      //             count: count,
      //           },
      //         ],
      //       };
          
      //       //console.log("====99===",data_obj);



      //        var indexx = data2.findIndex(obj => obj.id === data.id);
      //        if(indexx==-1)
      //        {

      //          console.log("=================00999========",userData);
      //          data2.push(userData);
      //         await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
      //        }
      //        else{


      //         console.log("=======87=======",data2[indexx].priduce_v)

      //         let arrr = data2[indexx].priduce_v;

            

             


      //         var veri_id = product.item.id;

      //         var indexx2 = arrr.findIndex(obj => obj.id === veri_id);

      //         var v_obj;
      //         if(indexx2==-1)
      //           {
      //            v_obj = data2[indexx].variants;

      //           console.log('====98======',v_obj);
  
      //           v_obj[[veri_id]] = count;
      //           var objj = {
      //             id: veri_id,
      //             price: product.item.sellingPrice,
      //             count: count,
      //           };
  
      //           console.log("====99===",objj);
  
      //           arrr.push(objj);

      //           data_obj = {
      //             name: data.name,
      //             variants: v_obj,
      //             id: data.id,
      //             priduce_v: arrr,
      //           };

      //           console.log("=======16========",data_obj);
  
      //           data2.splice(indexx, 1);
      //           data2.push(data_obj);
 
      //           console.log("============99888========",data2)
      //           await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
      //           }
      //         else{
      //           console.log('====9611======',arrr[indexx2]);
      //          /// v_obj = arrr[indexx2];
      //          //// v_obj['count'] = count;

      //          v_obj = data2[indexx].variants;

      //          console.log('====98======',v_obj);
 
      //          v_obj[[veri_id]] = count;

      //          let varrrrr=arrr[indexx2]
      //          varrrrr['count']=count;
               

      //           console.log('====96112======',varrrrr);
      //           data_obj = {
      //             name: data.name,
      //             variants: v_obj,
      //             id: data.id,
      //             priduce_v: [varrrrr],
      //           };


      //           console.log("=======160099========",data_obj);


      //           data2.splice(indexx2, 1);
      //           data2.push(data_obj);
 
      //           console.log("============99888========",data2)
      //           await AsyncStorage.setItem("addToCart", JSON.stringify(data2));
      //         }
  
               
                
  
               
              

             
      //        }


      //       console.log("=======11==null======",indexx);
      //     }



        ///  this.importData();


         this.importData();
      // }

      // this.setState(state => ({
      //   cartData: [...state.cartData, data_obj],
      // }));
    } catch (e) {
      console.log(e);
    }
  };

  onEndReached = ({distanceFromEnd}) => {
    console.log("sasssss--------------",this.state.offset)
    console.log("sasssss--------------",this.onEndReachedCalledDuringMomentum)
    if (!this.onEndReachedCalledDuringMomentum) {
      this.getproductlist(this.state.StoreID, this.state.CatId);
      this.onEndReachedCalledDuringMomentum = true;
    }
  };

  fetchCats = callType => {
    // this.importData();
    // if ((!this.state.nodata) && (!this.state.refreshing)) {
    // console.log(callType);
    this.setState({loading: true});

    WebService.GetData(`business-details/${this.state.StoreID}/categories`)
      // fetch('http://api.beta.diskounto.com/search/business?category='+this.state.category+'&lat=25.5940947&lng=85.1375645&formattedAddress=Patna%2C%20Bihar%2C%20India&city=Patna&state=BR&limit=20&skip='+this.state.offset)
      .then(CatRes => CatRes.json())
      .then(CaTResJson => {
        if (CaTResJson.length > 0) {
          this.setState({loading: false});
          if (callType == 'pullRefresh') {
            this.setState({cartegorydata: CaTResJson});
            this.getproductlist(this.state.StoreID, this.state.CatId);
          } else {
            this.setState({cartegorydata: CaTResJson, CatId: CaTResJson[0].id});
            this.getproductlist(this.state.StoreID, this.state.CatId);
          }
        } else {
          this.setState({cartegorydata: []});
        }
      })
      .catch(e => console.log(e));

    //}
  };
  getproductlist = (StoreID, CatId) => {

   console.log(this.state.CatId+'=====calling me===',CatId)
    this.setState({refreshing: true});
    let offsetValue=this.state.offset;
    if(this.state.CatId!=CatId)
          {
            offsetValue=0;
            this.setState({offset:0});
          }
          // `business-details/${StoreID}/categories/${CatId}?listProducts=true&skip=${offsetValue}&limit=2`,
    WebService.GetData(
      `business-details/${StoreID}/products?categoryId=${CatId}&skip=${offsetValue}&limit=7`,
    )
      .then(res => res.json())
      .then(resJson => {
        console.log("resJson------------------",resJson);
        if (resJson.length > 0) {
          if(this.state.CatId==CatId)
          {
            //this.onEndReachedCalledDuringMomentum = false;
            this.setState({data: [...this.state.data, ...resJson]});
          }else{
            this.setState({data:resJson,offset:0});
          }
          
         

          // console.log('product data.... ', resJson[0].products);
          // resJson[0].products.forEach(element => {
          //   this.setState({data: [...this.state.data, element]});
          // });

          // this.setState({
          //   offset: this.state.offset + 1,
          // });

          this.setState({
            CatId: CatId,
            refreshing: false,
            nodata: true,
            offset: this.state.offset + 1,
          });
        } else {
          this.setState({loadmore: false, refreshing: false});
        }
      })
      .catch(e => console.log(e));
  };

  removeDbync = async () => {
    //  const keys = await AsyncStorage.getAllKeys()
    let keys = [];
    try {
      const keys = await AsyncStorage.getAllKeys();

      console.log('sadsdsdsds-================', keys);

      const itemsArray = await AsyncStorage.multiGet(keys);

      itemsArray.map(item => {
        if (
          (`${item[0]}` == 'user' && `${item[0]}` == 'userToken') ||
          `${item[0]}` == 'SelectedStoreID' ||
          `${item[0]}` == 'RewordCart'
        ) {
          //console.log("llllllllllllllllllllllllllllllll")
        }
      });
    } catch (e) {
      // read key error
    }
    //// await AsyncStorage.removeItem('totalPrice')
    console.log();
  };

  importData = async () => {
    // this.setState({loadmore: true});
   //await AsyncStorage.removeItem("addToCart");
    try {
      this.state.localcart=[];

      const keys = await AsyncStorage.getAllKeys();

      const itemsArray = await AsyncStorage.multiGet(keys);
      let object = {};
      itemsArray.map(item => {
       // console.log('-------222----------', `${item[0]}`); //count
        if (`${item[0]}` == 'addToCart') {
           console.log("llllllllllllllllllllllllllllllll",item[1])
          //object['ubk'] = JSON.parse(item[1]);
          object['cart'] = JSON.parse(item[1]);

          //console.log("llllllllllllllllllllllllllllllll",object)
        } else {
        }
      });

      // const keys = await AsyncStorage.getAllKeys();
      // console.log('sadsdsdsds-================', keys);
      // const itemsArray = await AsyncStorage.multiGet(keys);
      // let object = {};
      // let object2 = {};
      // var p_count = 0;
      // var p_price = 0;
      // //var p_price =parseInt(

      // console.log('itemsArray.... ', itemsArray);

      // itemsArray.map(item => {
      //   if (
      //     `${item[0]}` == 'user' ||
      //     `${item[0]}` == 'userToken' ||
      //     `${item[0]}` == 'SelectedStoreID' ||
      //     `${item[0]}` == 'RewordCart' ||
      //     `${item[0]}` == 'Coordinate' ||
      //     `${item[0]}` == 'address'
      //   ) {
      //     // if (itemsArray.length == 3) {
      //     //   this.setState({ count: 0 });
      //     // }
      //   } else {
      //     if (`${item[0]}` == 'count') {
      //     } else if (`${item[0]}` == 'totalPrice') {
      //     } else {
      //       object[`${item[0]}`] = item[1];
      //       console.log(
      //         '===================1===============' + item[0],
      //         item[1],
      //       );

      //       var datta = JSON.parse(item[1]);

      //       if (datta == null) {
      //         console.log('===================1===============');
      //       } else {
      //         console.log('===================2===============');
      //         //}

      //         console.log('adadasdadasdas..........', datta);
      //         if (datta.hasOwnProperty('count')) {
      //           console.log('has count==== True');

      //           p_price =
      //             parseInt(p_price) +
      //             parseInt(JSON.parse(item[1]).price) *
      //               parseInt(JSON.parse(item[1]).count);
      //           p_count =
      //             parseInt(p_count) + parseInt(JSON.parse(item[1]).count);

      //           console.log('adadasdadasdas8888..........' + p_count, p_price);
      //           this.setState({
      //             totalPrice: p_price,
      //             count: p_count,
      //           });
      //         } else {
      //           console.log('has count==== false');
      //           datta.priduce_v.map(items => {
      //             console.log('adadasdadasdas8888..........', items);
      //             p_count = parseInt(p_count) + parseInt(items.count);
      //             p_price =
      //               parseInt(p_price) +
      //               parseInt(items.price) * parseInt(items.count);

      //             console.log(
      //               'adadasdadasdas8888..........' + p_count,
      //               p_price,
      //             );
      //             this.setState({
      //               totalPrice: p_price,
      //               count: p_count,
      //             });
      //           });
      //         }
      //       }
      //     }
      //   }
      // });

      // var count = Object.keys(object).length;
      // //  console.log('Object length===', count);
      // if (count == 0) {
      //   this.setState(state => ({
      //     localcart: [object],
      //     count: 0,
      //     totalPrice: 0,
      //   }));
      // } else {
        this.setState(state => ({
          localcart: [object],
        }));
      // }

      // return true;
    } catch (error) {
      console.error(error);
    }
  };

  variant_popup = async product => {
    // console.log("--------1---------------",product)

    this.setState({clickdata: product});

    this.bs.current.snapTo(0);
  };

  product_variant = data => {
    // console.log("======1=========",data.variants.length)

    if (data.variants.length == 1) {
      return (
        <View style={styles.productQuntity2}>
          {/* <Text style={styles.QunityText}>1L</Text> */}
        </View>
      );
    } else {
      return (
        <View style={styles.productQuntity1}>
          <Text>Customise</Text>
        </View>
      );
    }
  };

  checkCart = data => {
    try {
      let pid = data.id;

    //console.log("=========33333==========",data.variants.length)

   //console.log("-----999999---",this.state.localcart[0])
    //  if (this.state.localcart[0].hasOwnProperty(pid)) {
      ////  const userData = JSON.parse(this.state.localcart[0][pid]);

      if(data.variants.length >1)
      {

      //////  varient  
          ///   var avalibility = data;
          // console.log("ssdsdsds33333333",avalibility);
          return (
            <TouchableOpacity
              onPress={() => this.variant_popup(data)}
              style={{marginLeft: 20}}>
              <View style={styles.productAddToCart}>
                <View style={styles.ProductCartItem_variant}>
                  <Text
                    style={{
                      textAlign: 'center',
                      color: globalcolor.PrimaryColor,
                    }}>
                    Add{' '}
                  </Text>
                </View>
                <View style={styles.AddIcon}>
                  <FontAwesome
                    name="plus"
                    color={globalcolor.PrimaryColor}
                    size={20}
                  />
                </View>
              </View>
            </TouchableOpacity>
          );


        }
      else{
        //console.log("dfdfdfdfdfdfd");
        let arrr =this.state.localcart[0].cart;
      /// console.log("dfdfdfdfdfdfd",arrr);
        if (arrr === null || arrr === undefined) {

          return (
                    <TouchableOpacity
                    style={{marginLeft: 20}}
                     onPress={() => this.cutAdd(data, 1)}>
                      <View style={styles.productAddToCart}>
                        <View style={styles.ProductCartItem}>
                          <Text
                            style={{
                              textAlign: 'center',
                              color: globalcolor.PrimaryColor,
                            }}>
                            Add
                          </Text>
                        </View>
                        <View style={styles.AddIcon}>
                          <FontAwesome
                            name="plus"
                            color={globalcolor.PrimaryColor}
                            size={20}
                          />
                        </View>
                      </View>
                    </TouchableOpacity>
                  );

        }
        else{
 
         //console.log("dfdfdfdfdfdfd--1",pid);
        // console.log("dfdfdfdfdfdfd--1",arrr);

          var indexx = arrr.findIndex(obj => obj.id === pid);
          //arrr[indexx].priduce_v = count;
        //  console.log("sdsdsdsds---------",indexx);
  
  //var
               if(indexx!=-1)
               {
                // arrr[indexx].quantity = count;
                 /// console.log(indexx+'=================111.0', JSON.stringify(arrr[indexx].priduce_v[0].count));
                 //  console.log("dfdfdfdfdfdf------","--------"+arrr[indexx].priduce_v[0].count)
                   // parseInt(item.item.quantity)
                   var counttt=parseInt(arrr[indexx].priduce_v[0].count)
                  // console.log("fdfdfdfdfdf","....."+counttt);
                   // if (userData.hasOwnProperty('count')) {
                     return (
                       <View  style={{marginLeft: 25}}>
                       
                           <Counter
                             buttonStyle={{
                             borderColor: globalcolor.PrimaryColor,
                             borderWidth: 1,
                             fontSize: 20,
                           }}
                           buttonTextStyle={{
                             color: globalcolor.PrimaryColor,
                           }}
                           countTextStyle={{
                             color: globalcolor.PrimaryColor,
                           }}
                           max={data.maxOrderQuantity}
                           start={counttt}
                           onChange={(len, type) => {
                             console.log(len, type);
                           //  console.log('nooooooo', JSON.parse(cartData));

                             
                             this.cutAdd(data, len);
                             
                           }}
                         /> 
                       
                       </View>
                     );
               }
               else{

                return (
                  <TouchableOpacity
                  style={{marginLeft: 20}}
                   onPress={() => this.cutAdd(data, 1)}>
                    <View style={styles.productAddToCart}>
                      <View style={styles.ProductCartItem}>
                        <Text
                          style={{
                            textAlign: 'center',
                            color: globalcolor.PrimaryColor,
                          }}>
                          Add1
                        </Text>
                      </View>
                      <View style={styles.AddIcon}>
                        <FontAwesome
                          name="plus"
                          color={globalcolor.PrimaryColor}
                          size={20}
                        />
                      </View>
                    </View>
                  </TouchableOpacity>
                );


                //  return (
                //    <View style={{marginLeft: 25}}>
                   
                //        <Counter
                //        buttonStyle={{
                //          borderColor: globalcolor.PrimaryColor,
                //          borderWidth: 1,
                //          fontSize: 20,
                //        }}
                //        buttonTextStyle={{
                //          color: globalcolor.PrimaryColor,
                //        }}
                //        countTextStyle={{
                //          color: globalcolor.PrimaryColor,
                //        }}
                //        max={data.maxOrderQuantity}
                //        start={22}
                //        onChange={(len, type) => {
                //         /// console.log(len, type);
                //          this.cutAdd(data, len);
                //        }}
                //      /> 
                //    </View>
                //  );
               }
        }
      
      }
              
   
      //   } else {
      //     if (data.variants.length == 1) {
      //       // console.log("ssdsdsds11111")
      //       return (
      //         <View style={styles.productAddToCart}>
      //           <View style={styles.ProductCartItem}>
      //             <Text
      //               style={{
      //                 textAlign: 'center',
      //                 color: globalcolor.PrimaryColor,
      //               }}>
      //               Add{' '}
      //             </Text>
      //           </View>
      //           <View style={styles.AddIcon}>
      //             <FontAwesome
      //               name="plus"
      //               color={globalcolor.PrimaryColor}
      //               size={20}
      //             />
      //           </View>
      //         </View>
      //       );
      //     } else {
      //       // console.log("ssdsdsds33333333")
      //       return (
      //         <TouchableOpacity onPress={() => this.variant_popup(data)}>
      //           <View style={styles.productAddToCart}>
      //             <View style={styles.ProductCartItem}>
      //               <Text
      //                 style={{
      //                   textAlign: 'center',
      //                   color: globalcolor.PrimaryColor,
      //                 }}>
      //                 Add{' '}
      //               </Text>
      //             </View>
      //             <View style={styles.AddIcon}>
      //               <FontAwesome
      //                 name="plus"
      //                 color={globalcolor.PrimaryColor}
      //                 size={20}
      //               />
      //             </View>
      //           </View>
      //         </TouchableOpacity>
      //       );
      //     }
      //     //   console.log("44444444444444444")
      //   }
      // } else {
        
      //   console.log("44444444444444444")
      //   if (data.variants.length == 1) {
      //     // console.log("ssdsdsds11111",data.variants[0].active)

      //     if (data.variants[0].active) {
      //       return (
      //         <TouchableOpacity onPress={() => this.cutAdd(data, 1)}>
      //           <View style={styles.productAddToCart}>
      //             <View style={styles.ProductCartItem}>
      //               <Text
      //                 style={{
      //                   textAlign: 'center',
      //                   color: globalcolor.PrimaryColor,
      //                 }}>
      //                 Add
      //               </Text>
      //             </View>
      //             <View style={styles.AddIcon}>
      //               <FontAwesome
      //                 name="plus"
      //                 color={globalcolor.PrimaryColor}
      //                 size={20}
      //               />
      //             </View>
      //           </View>
      //         </TouchableOpacity>
      //       );
      //     } else {
      //       return (
      //         <View style={{width: '100%'}}>
      //           <View style={styles.productAddToCartDesible}>
      //             <View style={styles.ProductCartItemDesible}>
      //               <Text
      //                 style={{
      //                   textAlign: 'center',
      //                   color: globalcolor.desibleColour,
      //                 }}>
      //                 Add{' '}
      //               </Text>
      //             </View>
      //             <View style={styles.AddIcon}>
      //               <FontAwesome
      //                 name="plus"
      //                 color={globalcolor.desibleColour}
      //                 size={20}
      //               />
      //             </View>
      //           </View>
      //           <Text style={{color: '#FF0000', fontSize: 10, marginLeft: 20}}>
      //             Out of Stock
      //           </Text>
      //         </View>
      //       );
      //     }
      //   } else {
      //     ///console.log("ssdsdsds33333333")

      //     var avalibility = data;
      //     // console.log("ssdsdsds33333333",avalibility);
      //     return (
      //       <TouchableOpacity
      //         onPress={() => this.variant_popup(data)}
      //         style={{marginLeft: 10}}>
      //         <View style={styles.productAddToCart}>
      //           <View style={styles.ProductCartItem_variant}>
      //             <Text
      //               style={{
      //                 textAlign: 'center',
      //                 color: globalcolor.PrimaryColor,
      //               }}>
      //               Add{' '}
      //             </Text>
      //           </View>
      //           <View style={styles.AddIcon}>
      //             <FontAwesome
      //               name="plus"
      //               color={globalcolor.PrimaryColor}
      //               size={20}
      //             />
      //           </View>
      //         </View>
      //       </TouchableOpacity>
      //     );
      //   }
     // }
    } catch (e) {
      //     // saving error
    }
  };

  renderFooter = () => {
    return (
      <View style={styles.footer}>
        {this.state.loading ? (
          <ActivityIndicator
            size="large"
            color={globalcolor.PrimaryColor}
            style={{margin: 15}}
          />
        ) : null}
      </View>
    );
  };
  checkCartPopup = (data, product) => {
    try {
    // console.log("kkkkkkkkkkkkkkkkkk",product)
      let pid = product.id;
     // let v_id=data.item.id;
      //   console.log("================1================",v_id)
      //  //console.log("tttttt-------",this.state.localcart[0][pid])
        let arrr =this.state.localcart[0].cart;
       // console.log("dfdfdfdfdfdfd------333-",arrr);
    
       if (arrr === null || arrr === undefined) 
         {
          return (
            <TouchableOpacity
              onPress={() => {
                this.subCutAdd(data, product, 1);
              }}>
              <View style={styles.productAddToCart}>
                <View style={styles.ProductCartItem}>
                  <Text
                    style={{
                      textAlign: 'center',
                      color: globalcolor.PrimaryColor,
                    }}>
                    Add{' '}
                  </Text>
                </View>
                <View style={styles.AddIcon}>
                  <FontAwesome
                    name="plus"
                    color={globalcolor.PrimaryColor}
                    size={20}
                  />
                </View>
              </View>
            </TouchableOpacity>
          );
         }
         else{

        // }
      
        var indexx = arrr.findIndex(obj => obj.id === pid);
      // // //arrr[indexx].priduce_v = count;
    ///  console.log("sdsdsdsds---33------",indexx);
       if(indexx==-1)
       {
      //  console.log("bpppppppnooooo----1-")
        return (
          <TouchableOpacity
            onPress={() => {
              this.subCutAdd(data, product, 1);
            }}>
            <View style={styles.productAddToCart}>
              <View style={styles.ProductCartItem}>
                <Text
                  style={{
                    textAlign: 'center',
                    color: globalcolor.PrimaryColor,
                  }}>
                  Add{' '}
                </Text>
              </View>
              <View style={styles.AddIcon}>
                <FontAwesome
                  name="plus"
                  color={globalcolor.PrimaryColor}
                  size={20}
                />
              </View>
            </View>
          </TouchableOpacity>
        );
        
       }
      else{

        //console.log("bpppppppnooooo----2-",data.item.id)

       /// console.log("=================998888444================",this.state.localcart[0].cart[indexx])


        let v_array = this.state.localcart[0].cart[indexx].priduce_v;
        //console.log("==============44==================",v_array)
        var v_indexx = v_array.findIndex(obj => obj.id === data.item.id);

      // console.log("==============55==================",v_indexx)
       if(v_indexx==-1)
       {
       // console.log("==============No varient==================")
        return (
          <TouchableOpacity
            onPress={() => {
              this.subCutAdd(data, product, 1);
            }}>
            <View style={styles.productAddToCart}>
              <View style={styles.ProductCartItem}>
                <Text
                  style={{
                    textAlign: 'center',
                    color: globalcolor.PrimaryColor,
                  }}>
                  Add{' '}
                </Text>
              </View>
              <View style={styles.AddIcon}>
                <FontAwesome
                  name="plus"
                  color={globalcolor.PrimaryColor}
                  size={20}
                />
              </View>
            </View>
          </TouchableOpacity>
        );
       }
       else{
        //console.log("==============ok==================",v_array[v_indexx].count)

        return (
          <View>
            <Counter
              buttonStyle={{
                borderColor: globalcolor.PrimaryColor,
                borderWidth: 1,
              }}
              buttonTextStyle={{
                color: globalcolor.PrimaryColor,
                fontSize: 20,
              }}
              countTextStyle={{
                color: globalcolor.PrimaryColor,
                fontSize: 20,
              }}
              max={product.maxOrderQuantity}
              start={v_array[v_indexx].count}
              onChange={(len, type) => {
                console.log(len, type);
                this.subCutAdd(data, product, len);

              }}
            />
          </View>
        );

       }
    }

     

      }
    } catch (e) {
      //     // saving error
    }
  };

  renderItemComponent = data => {

    let imageUrl = '';
    if (data.item.photos.length > 0) {
      imageUrl = ConfigFile.ImageBaseUrl + data.item.photos[0].path;
    } else {
      imageUrl =
        'data:image/png;base64,';
    }

    return (
      <Animated.View
        style={{
          //```  margin: 20,
          opacity: Animated.add(0.1, Animated.multiply(this.fall, 1.0)),
        }}>
        <View style={styles.ProductListrow}>
          <View style={styles.ProductlistFirstItem}>
            <Image
              source={{uri: imageUrl}} //Change your icon image here
              style={[styles.ProductImage]}
            />
          </View>
          <View style={styles.ListBody}>
            <Text style={globalstyle.ProductText}>{data.item.name}</Text>

            {data.item.variants.map((value, index) =>
              value.isDefault == true ? (
                <Text style={globalstyle.ProductPrice} key={index}>
                  ₹
                  <Text style={{fontWeight: 'bold', color: '#000000'}}>
                    {' '}
                    {value.sellingPrice}
                  </Text>
                </Text>
              ) : null,
            )}
            <Text style={styles.Productdesc}>
              {data.item.description.substring(0, 65)}{' '}
            </Text>
            <View style={styles.ProductListrow}>
              {this.product_variant(data.item)}

              {this.checkCart(data.item)}
            </View>
          </View>
        </View>
      </Animated.View>
    );
  };

  product_variants = item => {
   /// console.log("================33============",item)
   /// console.log("================3311============",this.state.clickdata)

    
    return (
      <View>
        <View style={styles.ProductListrow}>
          <View style={(styles.ListBody, {width: '100%'})}>
          <Text style={globalstyle.ProductText}>{item.item.name}{item.id}</Text>
            <View
              style={
                (styles.ProductListrow, {width: '100%', flexDirection: 'row'})
              }>
              <View style={(styles.productQuntity, {flex: 1})}>
                <Text style={globalstyle.ProductPrice}>
                  ₹
                  <Text style={{fontWeight: 'bold', color: '#000000'}}>
                    {' '}
                    {item.item.sellingPrice}{' '}
                  </Text>
                  <Text
                    style={{
                      fontWeight: 'bold',
                      color: '#000000',
                      textDecorationLine: 'line-through',
                    }}>
                    ₹ {item.item.maximumPrice}{' '}
                  </Text>
                </Text>
              </View>
              <View
                style={{
                  flex: 1,
                  flexDirection: 'row',
                  justifyContent: 'flex-end',
                }}>
                {this.checkCartPopup(item, this.state.clickdata)}
              </View>
            </View>
          </View>
        </View>
      </View>
    );
  };

  ItemSeparator = () => (
    <View
      style={{
        height: 0.5,
        width: '100%',
        backgroundColor: '#C8C8C8',
      }}
    />
  );
  renderFooter = () => {
    return (
      <View style={styles.footer}>
        {this.state.loading ? (
          <ActivityIndicator
            size="large"
            color={globalcolor.PrimaryColor}
            style={{margin: 15}}
          />
        ) : null}
      </View>
    );
  };
  ActivityIndicatorShow() {
    return (
      <View>
        {this.state.loading ? (
          <ActivityIndicator
            size="large"
            color={globalcolor.PrimaryColor}
            style={{
              marginTop: 50,
              alignItems: 'center',
              justifyContent: 'center',
            }}
          />
        ) : null}
      </View>
    );
  }
  backToproduct = () => {
    console.log('Call me again.. productList');
  };
  handleRefresh = () => {
    this.importData();
    this.setState({refreshing: false}, () => {
      this.fetchCats('pullRefresh');
    }); // call fetchCats after setting the state
  };

  ImageDynamic = Data => {
    // console.log('ImageDynamic', Data.length);
    let ImgUrl = '';
    if (Data.length > 0) {
      ImgUrl = ConfigFile.ImageBaseUrl + Data[0].path;
      //globalcolor.ImageBaseUrl + value.photos[0].path
    } else {
      ImgUrl =
        'data:image/png;base64,';
    }
    return (
      <Image
        source={{uri: ImgUrl}} //Change your icon image here
        style={styles.itemPhoto}
      />
    );
  };

  footerViewCart() {
    return this.state.count > 0 ? (
      <View style={styles.footer}>
        <View style={styles.footerItem}>
          <Text style={{textAlign: 'left', color: '#ffffff', marginLeft: 10}}>
            {this.state.count} Item | ₹ {this.state.totalPrice}
          </Text>
        </View>
        <TouchableOpacity
          style={{flex: 1}}
          onPress={() => {
            this.props.navigation.navigate('AddToCart', {
              StoreID: this.state.StoreID,
              acceptsCOD: this.state.acceptsCOD,
              requireSlot: this.state.requireSlot,
              totalPrice: this.state.totalPrice,
              count: this.state.count,
              // CallProdutList: this.backToproduct.bind(this),
            });
          }}>
          <View style={{padding: 10}}>
            <Text style={{textAlign: 'right', color: '#ffffff'}}>
              View Cart
              {/* <FontAwesome name="plus" color={globalcolor.Textcolor} size={20} style={{ marginLeft: 10 }} /> */}
            </Text>
          </View>
        </TouchableOpacity>
      </View>
    ) : null;
  }

  render() {

    if (this.state.cartegorydata.length > 1) {
      return (
        <SafeAreaView
          style={{
            flex: 1,
            zIndex: 2,
            marginTop: 40,
            marginLeft: 5,
            marginRight: 5,
          }}>
          <View style={[globalstyle.serachBar, globalstyle.BorderSearchbar]}>
            <View>
              <Image
                source={require('../assets/img/SearchIcon.png')} //Change your icon image here
                style={globalstyle.SearchIcon}
              />
            </View>
            <TextInput
              placeholder="Search Product"
              style={globalstyle.SearchBox}
              onChangeText={val => this.SearchProduct(val)}
            />
          </View>
          <View>
            <ScrollView
              horizontal={true}
              showsHorizontalScrollIndicator={false}>
              {this.state.cartegorydata.map((value, index) => (
                <TouchableOpacity
                  style={styles.ProductListrow}
                  key={index}
                  onPress={() => {
                    this.getproductlist(this.state.StoreID, value.id);
                  }}>
                  <View
                    style={[
                      styles.item,
                      value.id == this.state.CatId
                        ? {borderColor: globalcolor.PrimaryColor}
                        : {borderColor: globalcolor.Separator},
                    ]}>
                    {this.ImageDynamic(value.photos)}
                    {/* <Image
                      source={{ uri: globalcolor.ImageBaseUrl + value.photos[0].path }}
                      style={styles.itemPhoto}
                    /> */}
                    <Text
                      style={{
                        textAlign: 'center',
                        fontFamily: globalstyle.Font,
                        fontSize: 11,
                      }}>
                      {value.name.substring(0, 10)}
                    </Text>
                  </View>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>

          <FlatList
            data={this.state.data}
            keyExtractor={item =>
              item.id.toString() + Math.floor(Math.random() * 10000000000000)
            }
            ItemSeparatorComponent={this.ItemSeparator}
            renderItem={item => this.renderItemComponent(item)}
            ListFooterComponent={this.renderFooter}
            refreshing={this.state.refreshing}
            onRefresh={this.handleRefresh}
            onEndReached={this.onEndReached.bind(this)}
            onEndReachedThreshold={0.5}
            onMomentumScrollBegin={() => {
              this.onEndReachedCalledDuringMomentum = false;
            }}
          />
          {this.footerViewCart()}

          <BottomSheet
            ref={this.bs}
            snapPoints={[330, 0]}
            renderContent={this.renderInner}
            renderHeader={this.renderHeader}
            initialSnap={1}
            callbackNode={this.fall}
            enabledGestureInteraction={true}
          />
        </SafeAreaView>
      );
    } else {
      return (
        <View style={globalstyle.ActivityContainer}>
          {this.ActivityIndicatorShow()}
          <Text style={globalstyle.ActivityIndicator}>
            Loading please wait....
          </Text>
        </View>
      );
    }
  }
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    // justifyContent: "space-between",
    //  backgroundColor: "#FFF5EA",
    // padding: 20,
    //  margin: 10,
  },
  footer: {
    width: '100%',
    height: 50,
    zIndex: 1,
    position: 'absolute',

    flex: 1,
    flexDirection: 'row',
    backgroundColor: globalcolor.PrimaryColor,
    alignItems: 'center',
    position: 'absolute', //Here is the trick
    bottom: 0, //Here is the trick
  },
  footerItem: {
    flex: 0.5,
    textAlign: 'left',
    color: '#ffffff',
  },
  footerItem2: {
    flex: 1,
    borderWidth: 2,
    alignSelf: 'center',
    padding: 20,
    alignSelf: 'center',
    // alignItems: 'flex-end',
    //textAlign: 'right',
    // color: '#ffffff',
  },
  item: {
    marginLeft: 5,
    marginRight: 5,
    borderWidth: 1,
    width: 60,
    height: 60,
    borderRadius: 10,
    // marginBottom:60
  },
  productAddToCartDesible: {
    borderWidth: 1,
    flexDirection: 'row',
    width: '35%',
    height: 30,
    borderColor: globalcolor.desibleColour,
    marginLeft: 15,
  },
  productAddToCartDesibleVariat: {
    borderWidth: 1,
    flexDirection: 'row',
    width: '55%',
    height: 30,
    borderColor: globalcolor.desibleColour,
    marginLeft: 15,
  },
  itemPhoto: {
    width: 40,
    height: 40,
    justifyContent: 'center',
    alignSelf: 'center',
    marginVertical: 1,
  },

  bottomNavigationView: {
    backgroundColor: '#fff',
    width: '100%',
    height: 250,
    justifyContent: 'center',
    alignItems: 'center',
  },
  itemText: {
    color: 'rgba(255, 255, 255, 0.5)',
    // marginTop: 5,
  },
  ProductlistFirstItem: {
    padding: 5,
    width: '25%',
  },
  ListBody: {
    padding: 5,
    marginLeft: 10,
    width: '80%',
  },
  ListSecondIcon: {
    width: '15%',
  },
  ProductListrow: {
    //marginLeft:20,
    flex: 1,
    //marginRight:20,

    flexDirection: 'row',
    //marginTop:10,
    // marginBottom:
  },
  ProductImage: {
    height: 80,
    width: 80,
    borderRadius: 8,
  },
  SlugListText: {
    fontFamily: globalcolor.Font,
    color: globalcolor.SeconderFontColor,
    marginTop: 2,
    fontWeight: 'bold',
  },
  Productdesc: {
    fontFamily: globalcolor.Font,
    color: globalcolor.SeconderFontColor,
  },

  productQuntity2: {
    flexDirection: 'row',
    width: '35%',
    height: 30,
    marginLeft: 5,
    marginRight: 7,
  },

  productQuntity1: {
    // borderWidth:1,
    flexDirection: 'row',
    width: '35%',
    height: 30,
    borderColor: globalcolor.borderColor,
    marginRight: 7,
  },
  productQuntity: {
    height: 30,
    borderColor: globalcolor.borderColor,
    flexDirection: 'row',
  },
  productQuntityItem: {
    marginLeft: 2,
    width: '70%',
    height: 30,
    justifyContent: 'center',
    alignSelf: 'center',
    borderEndWidth: 1,
  },

  productAddToCart: {
    borderWidth: 1,
    flexDirection: 'row',
    width: '72%',
    height: 35,
    borderColor: globalcolor.PrimaryColor,
    marginLeft: 15,
  },
  productAddToCartVariatActive: {
    borderWidth: 1,
    flexDirection: 'row',
    width: '80%',
    height: 30,
    borderColor: globalcolor.PrimaryColor,
    marginLeft: 15,
  },
  productAddToCartVariant: {
    borderWidth: 1,
    flexDirection: 'row',
    width: '50%',
    height: 30,
    borderColor: globalcolor.PrimaryColor,
    marginLeft: 15,
  },
  ProductCartItemDesible: {
    marginLeft: 2,
    width: '60%',
    height: 30,
    justifyContent: 'center',
    alignSelf: 'center',
    borderEndWidth: 1,
    borderEndColor: globalcolor.desibleColour,
  },
  ProductCartItem: {
    marginLeft: 2,
    width: '60%',
    height: 30,
    justifyContent: 'center',
    alignSelf: 'center',
    borderEndWidth: 1,
    borderEndColor: globalcolor.PrimaryColor,
  },
  ProductCartItem_variant: {
    marginLeft: 2,
    width: '63%',
    height: 35,
    justifyContent: 'center',
    alignSelf: 'center',
    borderEndWidth: 1,
    borderEndColor: globalcolor.PrimaryColor,
  },
  ProductCartItem2: {
    marginLeft: 2,
    width: '40%',
    height: 30,
    justifyContent: 'center',
    alignSelf: 'center',
    borderEndWidth: 1,
    borderLeftWidth: 1,
    borderEndColor: globalcolor.PrimaryColor,
    borderLeftColor: globalcolor.PrimaryColor,
  },
  ProductCartItem_minus: {
    width: '30%',
    height: 30,

    justifyContent: 'center',
    alignSelf: 'center',
    borderEndWidth: 1,
    borderEndColor: globalcolor.PrimaryColor,
  },
  AddIcon: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    width: '100%',
    fontFamily: globalcolor.Font,
  },
  QunityText: {
    fontFamily: globalcolor.Font,
  },

  panel: {
    backgroundColor: '#FFFFFF',
    borderWidth: 1,
    zIndex: 1,
    //height: '100%',
    borderColor: globalcolor.PrimaryColor,
    borderRadius: 10,
    padding: 10,
    marginTop: 33,
  },

  header: {
    shadowOffset: {width: -1, height: -3},
    shadowRadius: 2,
    shadowOpacity: 0.4,
    // elevation: 5,
    paddingTop: 20,
    borderTopLeftRadius: 20,
    borderTopRightRadius: 20,
  },
  panelHeader: {
    alignItems: 'center',
  },
  panelHandle: {
    width: 40,
    height: 8,
    borderRadius: 4,
    backgroundColor: globalcolor.PrimaryColor,
  },
  panelTitle: {
    fontSize: 14,

    height: 50,
    fontFamily: globalcolor.Font,
    fontWeight: 'bold',
  },

  panelSubtitle: {
    fontSize: 14,
    color: 'gray',
    height: 30,
    //marginBottom: 10,
  },
  panelButton: {
    padding: 13,
    borderRadius: 10,
    backgroundColor: '#FF6347',
    alignItems: 'center',
    marginVertical: 7,
  },
  panelButtonTitle: {
    fontSize: 17,
    fontWeight: 'bold',
    color: 'white',
  },
  FlatListStyle: {
    flex: 1,
    zIndex: 1,
    marginTop: 20,
  },
});
