...
export default class CheckOutAdderss extends Component {
  ...
  DeliveryCheck(Address) {
    ...
    let requestOptions = {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ businessId: this.state.StoreID, items: this.state.localcart, address: Address, checkoutOption: "DELIVERY" })
    };
    ...
    WebService.PostData('cart-details', requestOptions)
      ...
      .then(resJson => {
        ...
        this.setState({ isDeliverable: resJson.isDeliverable, DeliverablePaymentDeatils: resJson.pricing });
  ...
  ConfirmOrder(data) {
    ...
    if (data.dateTime != "") {
      ...
      if (data.payment == "PREPAID") {
        OrderText = 'PAY NOW ( ₹' + this.state.DeliverablePaymentDeatils.customerToPay + ' )';
      } else {
        OrderText = 'PLACE ORDER ( ₹' + this.state.DeliverablePaymentDeatils.customerToPay + ' )';
      }
      ...
      this.state.deliveryType == 'DELIVERY' ?
        ...
        <View style={styles.card}>
          ...
          <View style={{ flex: 1, flexDirection: 'row', marginTop: 10 }}>
            ...
            <View style={{ flex: 0.8 }}>
              <Text> Ghar Ka Bazaar , Kurji , opposite Mohan Sweets , Patna - 800013</Text>
            </View>
          </View>
      ...
      this.state.isDeliverable ?
        <View style={styles.card}>
          ...
          <View style={styles.PaymentDetailsRow}>
            ...
            <View style={{ flex: 0.5 }}>
              <Text style={{ textAlign: 'right' }}>₹ {this.state.DeliverablePaymentDeatils.totalPrice}</Text>
            </View>
          </View>
          {/* <View style={styles.PaymentDetailsRow}>
            <View style={{ flex: 0.5 }}>
          ...
          <View style={styles.PaymentDetailsRow}>
            ...
            <View style={{ flex: 0.5 }}>
              <Text style={{ textAlign: 'right' }}>₹ {this.state.DeliverablePaymentDeatils.transactionCharge.amount + this.state.DeliverablePaymentDeatils.transactionCharge.gst}</Text>
            </View>
          </View>
          <View style={[styles.PaymentDetailsRow]}>
            <View style={{ flex: 0.5 }}>
              <Text style={{ fontWeight: 'bold', fontSize: 16 }}>Amount to Pay</Text>
            </View>
            <View style={{ flex: 0.5 }}>
              <Text style={{ textAlign: 'right', fontWeight: 'bold' }}>₹ {this.state.DeliverablePaymentDeatils.customerToPay} </Text>
            </View>
          </View>
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView>
          <View style={[styles.container], { flex: 1, padding: 1, marginTop: 50 }}>
